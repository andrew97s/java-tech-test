    ...
    public void init() throws InterruptedException, SSLException {
        ...
        if (config.isUseEventExecutorGroup()) {
            eventExecutorGroup = new DefaultEventExecutorGroup(
                    config.getEventExecutorGroupThreads() == 0 ? 16 : config.getEventExecutorGroupThreads(),
                    new DefaultThreadFactory("netty-ws-event-pool")
            );
        }
        // boss 线程池数量
        EventLoopGroup boss = new NioEventLoopGroup(
                config.getBossLoopGroupThreads() ,
                new DefaultThreadFactory("netty-boss-pool")
        );
        // worker 线程池数量
        EventLoopGroup worker = new NioEventLoopGroup(
                config.getWorkerLoopGroupThreads(),
                new DefaultThreadFactory("netty-worker-pool")
        );
